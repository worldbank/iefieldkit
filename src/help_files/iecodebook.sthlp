...
The default behavior is to retain all variables. {bf:Alternatively, to drop variables one-by-one, write . (a single period) in the "name" (or "choices") column of the codebook.}
Unused value labels are always dropped by {cmdab:iecodebook}.{p_end}
{break}
